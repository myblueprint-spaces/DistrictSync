...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` — folders section (PickerFields + Dropdown; Save gated on `setup_state(...).can_save`) + `_build_schedule_section` (run-time + run-as caption + password field → gated Register → `validate_run_time` raise-gate → `register_task`/`register_cron` UNCHANGED → verdict via `classify_schedule_error`; password local, sole sink `run_as_password=`, I1/I3). SFTP: IA-4b.
