...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds the themed window + `dict[id -> placeholder factory]` + delegates the rail to `nav_rail.build_nav` (id-keyed `select_by_id`); swaps `setup`→folders, `home`→`build_home`, `convert`→`build_convert` (IA-5a) unconditionally; owns content host + zero-orphan close + `_on_leave` seam (write-guard is IA-5b).
...
- `src/ui_flet/screens/convert.py` — View glue (coverage-omitted): `build_convert(page)` — manual convert (`PickerField` + `FileChip`s vs `extract_required_files` + "Convert now" → `JobRunner.run` → verdict + `metric_tile`s + quality). Holds `convert_job(config, input_dir, *, anomaly_ack=False)` — parity-locked adapter (`load_from_bytes`→`run_transform`→`compute_anomalies`→[NEEDS_ANOMALY_ACK, no write until ack]→`save_all`) off-thread. IA-5a.
