...
- `src/ui_flet/setup_flow.py` — COUNTED pure wizard state machine (no flet/IO — D8/Slice 8): five `SetupStep`s + `derive_flow(FlowInputs)->FlowState` (resume = first unsatisfied step from injected state, no cursor; `can_finish` never sets `setup_completed`) + `can_advance` gate, `DeliveryFact`, step helpers, byte-exact `finish_copy` (3 variants), `TRANSITION_CUE`, `task_args_changed`, `auto_selected_district` (iff one config, D9).
