...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds themed window + `dict[id -> factory]` + rail via `nav_rail.build_nav`; HOLDS the rail handle and `select_by_id` syncs `rail.selected_index` so programmatic nav moves the highlight too (D7); swaps `setup`/`home`/`convert`/`run_history`/`mapping`/`help` placeholders (`help` precedes the `DISTRICTSYNC_UI_DEMO` override); owns content host + zero-orphan close.
