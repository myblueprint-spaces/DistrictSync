...
- `src/ui_flet/picker_field.py` — View glue (coverage-omitted): `PickerField` (themed `ft.Column`) — label + "Browse…" `FilledButton` calling the async `pick_directory` wrapper + chosen-path display + inline valid/invalid line; takes a `validator` + `on_change(path, result)`; the one reusable picker every later surface reuses (no tkinter port).
...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` — Setup folders surface (two `PickerField`s + SIS/district `ft.Dropdown` from `loader.available_configs()` showing `district_name`); Save is structurally disabled until `setup_state(...).can_save`, persists `input_dir`/`output_dir`/`sis_type` → `is_complete()`; bound into `shell.build_screens` via `functools.partial`.
