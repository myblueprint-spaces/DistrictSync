...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds themed window + `dict[id -> factory]` + rail via `nav_rail.build_nav` (id-keyed `select_by_id`); swaps `setup`/`home`/`convert`/`run_history` placeholders; owns content host + zero-orphan close + `_on_leave` seam reading `convert.is_write_in_flight()` (IA-5b write-guard — logs only, NEVER blocks the atomic close).
