...
- `src/sftp/uploader.py` — `SFTPUploader`: paramiko SSHClient to an `ALLOWED_SFTP_HOSTS` host; passwords via OS keyring (`KEYRING_SERVICE`); zips the **rostering** CSVs into `districtsync_<sis>_YYYY-MM-DD.zip` and uploads any `StudentAttendance.csv` standalone outside the zip; `test_connection(password_override=None)` (typed pw → `client.connect()` ONLY, never keyring/logs/message; D6), `upload_csvs()`, `get_stored_password()`.
...
- `src/ui_flet/sftp_copy.py` — COUNTED pure SFTP Test-connection trust copy (no flet): `sftp_test_copy(provenance, unsaved_edits, host, username)` (stored vs typed credential; never over-claims the nightly sync for unsaved values) + `sftp_form_differs_from_saved(cfg, ...)` (unsaved-edits predicate, port normalized). D6/Slice 7.
...
- `src/ui_flet/nav_rail.py` — View glue (coverage-omitted): `build_nav(..., attention_ids) -> (view, rail)` — flat `ft.NavigationRail` (fixed `ordered`, brand mark + reassurance above Exit); returns the rail handle for highlight sync (D7) + `badge` mutation; `attention_badge()` = the D4 Setup dot. Selection by `dest.id`; initial index via `nav.selected_index_for`.
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds the themed window (brand `page.window.icon`), `dict[id->factory]`, rail (`nav_rail.build_nav`); `select_by_id` syncs `rail.selected_index` (D7); OFF-thread `schedule_probe`+`needs_setup_badge` raises the Setup badge on a missing/contradicted schedule (D4, Windows-only); zero-orphan close via async `_close_window`.
...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` = folders (gated Save) + `_build_schedule_section` (`register_task`/`register_cron`→`classify_schedule_error`; D4: OFF-thread readout, **Unregister**, save-after-success, transient warning) + `_build_sftp_section` (allowlist + keyring; Test off-thread + side-effect-free, provenance copy (`sftp_copy`), keyring on Save only — D6). Enter-to-submit via `setup_gates`.
