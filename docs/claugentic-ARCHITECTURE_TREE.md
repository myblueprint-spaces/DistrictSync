...
- `src/ui_flet/screens/convert.py` — View glue (coverage-omitted): `build_convert(page)` manual convert (`PickerField`+`FileChip`s → `JobRunner.run` → verdict/tiles/quality; SFTP pre-flight → exit-3 `BUILT_NOT_DELIVERED`) + `convert_job(...)` adapter (ack-gated `save_all` → `_record_manual_run` store `source="manual"`). Slice 9: `convert_output` run-gate + output caption + `_output_folder_row`; `convert_job` FAILS LOUD on unset output. IA-5a/b.
