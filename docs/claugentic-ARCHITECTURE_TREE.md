...
- `src/ui_flet/home_status.py` — COUNTED pure Home status-derivation (no flet): `HomeStatus`/`HomeMetrics`/`FixAction` + `is_stale` + `derive_home_status` (verdict rules) + shared precedence (`LatestReason`/`classify_latest_reason`/`verdict_for_reason`). Owns the entity vocabulary: `_ROSTERING_ENTITIES`/`_MYBLUEPRINT_ENTITIES` + SINGLE-SOURCE `ENTITY_LABELS` (entity→output-CSV label) used by run_table/home/convert/mapping. IA-3a/IA-6/IA-8a.
