...
- `src/scheduler/windows.py` — Windows Task Scheduler: `register_task` runs a FIXED PowerShell `Register-ScheduledTask` script (`_build_register_script()` via `-EncodedCommand`, reads only `$env:DSYNC_*`); the action carries `--source scheduled` (`_build_action_args`, store source tag); password→`Password`/`Highest`, else `Interactive`/`Limited` (never S4U); errors de-CLIXML'd; `is_elevated()`; `delete_task`/`query_task` on `schtasks.exe`.
...
- `src/ui_flet/home_status.py` — COUNTED pure Home status-derivation (no flet): `HomeStatus`/`HomeMetrics`/`FixAction` + `is_stale` + `derive_home_status` (consumes `store.read_run_records` + a `store_created_at` signal for the honest fresh-start empty state) + shared precedence (`LatestReason`/`classify_latest_reason`/`verdict_for_reason`). Owns the entity vocabulary + SINGLE-SOURCE `ENTITY_LABELS`. IA-3a/IA-6/IA-8a/IA-9.
...
- `src/ui_flet/run_history.py` — COUNTED pure PII-free Run-History derivation (no flet; 2nd consumer of `store.read_run_records`+`is_stale`): `HistoryBanner`/`SftpDelivery`/`RunRow` (NO `error`) + `derive_history_banner` (None→unavailable, []→no-runs / fresh-start via `store_created_at`, else classify latest via `home_status`, CLEAN+stale→WARNING) + `to_run_row(s)`. NEVER raw `error`/`ANOMALY:`. IA-6/IA-9.
...
- `src/ui_flet/screens/home.py` — View glue (coverage-omitted): `build_home(page, *, app_config, on_navigate)` three-way Home. (a) `nav.needs_setup`→`build_onboarding`; (b)/(c) read `store.read_run_records` (+ `store_meta` on empty for fresh-start) + `derive_home_status`, render verdict-first (`HealthVerdictBanner` + fix-path button + `metric_tile`). Sync read; never-crash `ErrorCard`. IA-3b.
- `src/ui_flet/screens/convert.py` — View glue (coverage-omitted): `build_convert(page)` manual convert (`PickerField`+`FileChip`s → `JobRunner.run` → verdict/tiles/quality; SFTP pre-flight → exit-3 `BUILT_NOT_DELIVERED`) + `convert_job(...)` off-thread adapter (`run_transform`→`compute_anomalies`→ack-gated `save_all`→tight `upload_csvs` catch → `_record_manual_run` writes the store `source="manual"`) + `is_write_in_flight()`. IA-5a/b.
- `src/ui_flet/screens/run_history.py` — View glue (coverage-omitted): `build_run_history(page, *, app_config)` — read-only Run History (no `on_navigate`). Sync read (`history.store.read_run_records` + `store_meta` on the empty branch); verdict-first `derive_history_banner` → `HealthVerdictBanner`, then (non-empty) `run_table(to_run_rows(records)[:50])` in a horizontally-scrollable `ft.Row`. None/[]→banner only; never-crash `ErrorCard`. IA-6.
