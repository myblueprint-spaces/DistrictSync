...
- `src/scheduler/windows.py` — Windows Task Scheduler: `register_task` = FIXED `Register-ScheduledTask` script (`-EncodedCommand`, only `$env:DSYNC_*`, action `--source scheduled`, de-CLIXML'd errors); `is_elevated()`. `read_schedule -> ScheduleReadback` (D4): tri-state read-back (`Get-ScheduledTask`+`Info`, ~10s timeout) — not-found→`found=False`, other/non-Windows→`found=None`; `delete_task` on `schtasks`.
...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` = folders (gated Save) + `_build_schedule_section` (`register_task`/`register_cron`→`classify_schedule_error`; D4: OFF-thread live readout, **Unregister** (`interpret_unregister`), save-after-success, transient-location warning) + `_build_sftp_section` (allowlist + keyring; Test off-thread). Enter-to-submit via `setup_gates` (Slice 2).
