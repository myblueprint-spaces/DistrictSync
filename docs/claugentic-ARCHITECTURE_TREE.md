...
- `src/ui_flet/nav_rail.py` — View glue (coverage-omitted): `build_nav(*, ordered, selected_id, on_select, on_exit)` — the flat state-aware `ft.NavigationRail` (brand mark `leading` + one destination per `ordered` entry + Exit `trailing`), reordered so the prominent group leads (option (a): no headers, built-in a11y). Owns NO lifecycle + NO config; selection by `dest.id`, native highlight. The F6 rail-view extraction from the shell (plan 0018).
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds the themed window + a `dict[id -> placeholder factory]` of branded placeholders + delegates the rail to `nav_rail.build_nav` (reorder via `nav.ordered_destinations`, initial `nav.prominent_initial_id`, id-keyed `render_by_id`/`select_by_id`); owns window + content host + zero-orphan close + `_on_leave` seam; `DISTRICTSYNC_UI_DEMO` routes Help to the demo.
...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` — Setup folders surface (two `PickerField`s + SIS/district `ft.Dropdown` from `loader.available_configs()` showing `district_name`, header/form via the components `card`); Save (`primary_button` + `disabled_bgcolor`) structurally disabled until `setup_state(...).can_save`, persists input/output/SIS → `is_complete()`; mounted via `shell.build_screens`.
