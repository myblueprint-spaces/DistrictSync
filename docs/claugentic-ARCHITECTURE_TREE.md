...
- `src/ui_flet/nav.py` — Pure nav-state model (no flet import): `Destination`/`NavGroup`/`DESTINATIONS` + `nav_model(AppConfig)` (grouped destinations + the prominent group: Get-started until `is_complete()` + `schedule_registered`, then Everyday) + total render-ordering helpers `ordered_destinations` (flat, prominent group first, empties dropped) + `prominent_initial_id` (launch selection) — rendered by `nav_rail` (IA-1).
...
- `src/ui_flet/humanize.py` — COUNTED pure helper (no flet import): `friendly_district_name(sis_type, *, config_dir=None)` maps a SIS id → its human `district_name` via `loader.load_config`; TOTAL — empty→`""`, unknown/broken/empty-name→raw id (warned, never raises). Single source of the id→name mapping (DS-2); `config_dir` is the `load_config` test seam. Used by onboarding + `screens/setup`.
