...
- `src/etl/pipeline.py` — Core ETL orchestration (`run_pipeline`, `run_transform`, `extract_required_files`, `compute_anomalies`): coordinates Extractor → Transformer → Loader, anomaly detection, diff, quality report, structured JSON run-log, and optional SFTP upload; `run_transform(raw_data, mappings, global_config) -> TransformOutputs` (NamedTuple of `outputs` + `field_orders` + `data_errors`) is the shared transform-orchestration seam — school-year determination + `enabled_entities`/`entity_order` filtering + per-entity loop + field-order collection + fail-loud field-transform error ledger — called by `run_pipeline`; `run_pipeline` fails loud (raise → exit 1) when NO usable required input was loaded (every required file missing/empty) and surfaces any `data_errors` into a consolidated ERROR + the run-log `data_errors` summary (ETL `status` stays `success` — a separate axis from delivery/data errors); `compute_anomalies(outputs, output_dir) -> list[str]` is the single-sourced row-count-drop compute (plain `ANOMALY_THRESHOLD`-driven warning strings, no logging) consumed by both the CLI (`_check_anomalies` thin wrapper adds the `ANOMALY:` prefix + `logger.warning`) and the Convert page (`st.warning`); returns `PipelineResult` dataclass (`entity_counts`, `sftp_attempted`, `sftp_ok`, `anomalies`); logs ERROR and propagates failure so `main` can exit 3 on SFTP failure; importable by UI and tests.
- `src/etl/loader.py` — `DataLoader`: writes DataFrames as UTF-8 CSVs with YAML field ordering — BOM (`utf-8-sig`) for Excel, except `_NO_BOM_ENTITIES` (`StudentAttendance`) written plain UTF-8 for SpacesEDU's BOM-strict parser; `csv_encoding(entity)` is the single source of the BOM rule and `select_ordered(df, field_order, entity)` the single source of column selection + the fail-loud missing-column `ValueError` (both shared by the disk write path and the UI download/zip path, so the two never diverge); `save_all()` stages to `.tmp_<timestamp>/` then commits via `_commit_staged` with **backup-and-restore atomicity** — each existing target is moved aside into `.bak_<timestamp>/` and the staged file promoted with `os.replace` (atomic same-fs overwrite); any mid-commit failure rolls back (restore originals / remove new files) before the `finally` cleans both dirs, so the output dir is never left torn.
...
- `src/etl/transformers/base.py` — `BaseTransformer` ABC with utilities shared by all entity transformers: CEDS grade mapping, `ALLOWED_TRANSFORMS` security allowlist, `apply_field_map()` (row-resilient + fail-loud: a row whose transform raises blanks only that cell while valid rows keep their value; an unknown-transform / column-level error blanks the column; all failures recorded to `context.data_errors` + logged ERROR — never silently swallowed, never fails the run), `assign_class_ids()`, excluded-course-code filters, the config-driven active-student predicate (`resolve_active_config()` / `is_active_mask()` / `past_withdraw_date()`), and `filter_to_active()` (zero-orphan roster filter shared by Classes + Enrollments).
...
- `src/ui/pages/02_Convert.py` — Ad-hoc conversion page: a thin adapter over the shared ETL engine — uploaded GDE bytes → `DataExtractor.load_from_bytes` → `run_transform` (from `src/etl/pipeline.py`) → writes via `DataLoader` (download/zip + SFTP both call `DataLoader.csv_encoding`, so the per-entity BOM rule and column order match the CLI byte-for-byte); renders quality report, diff vs previous output, and offers ZIP download and optional SFTP upload.
- `src/ui/pages/03_Run_History.py` — Run History page: parses `__DISTRICTSYNC_RUN__` JSON log lines from `~/.districtsync/etl_tool.log` into a tabular view; falls back to raw log tail when no structured lines exist. The honest at-a-glance Status cell (display-only, derived from the same booleans, no exit/run-log change) shows amber "ETL OK · SFTP FAILED" when ETL succeeded but delivery failed and "Completed with N data errors" when the run carried field-transform errors, so the headline Status never contradicts the exit code.
