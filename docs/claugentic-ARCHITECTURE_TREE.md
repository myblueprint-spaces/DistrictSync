...
- `src/ui_flet/home_status.py` — COUNTED pure Home status-derivation (no flet; trust core): `FixAction`/`HomeMetrics`/`HomeStatus` + `STALE_AFTER_HOURS=36` + `is_stale` + `derive_home_status(...)` (None→unavailable, []→empty, failed-ETL/SFTP→FAILED, anomaly/data-errors/stale→WARNING, else HEALTHY). Also the SHARED precedence IA-6 reuses: `LatestReason` + `classify_latest_reason` + `verdict_for_reason`. NEVER surfaces `error`. IA-3a/IA-6.
- `src/ui_flet/humanize.py` — COUNTED pure helpers (no flet): `friendly_district_name(sis_type, *, config_dir=None)` maps a SIS id → its `district_name` (empty→`""`, unknown/broken→raw id); `friendly_timestamp(iso, *, now=None)` → a plain phrase ("just now"/"5 hours ago"/"yesterday at 3:00 AM"). Both TOTAL — never raise/surface the raw id or ISO (bad ts→"recently"). DS-2 + IA-3. Used by onboarding, setup, `home_status`.
...
- `src/ui_flet/convert_result.py` — COUNTED pure PII-free Convert-result model (no flet): `ConvertStatus` (DELIVERED / BUILT_NOT_DELIVERED [exit-3] / BUILT_WITH_DATA_ERRORS / NEEDS_ANOMALY_ACK / NO_INPUT / NO_OUTPUT) + frozen `ConvertResult` (counts/booleans/quality — NO DataFrames) + total `summarize -> (Verdict, headline, detail)`; NEVER interpolates a raw path/`sis_type`/column. IA-5a.
- `src/ui_flet/run_history.py` — COUNTED pure PII-free Run-History derivation (no flet; 2nd consumer of `read_run_records`+`is_stale`): `HistoryBanner` + `SftpDelivery` + frozen `RunRow` (NO `error` field) + `derive_history_banner(...)` (None→unavailable, []→no-runs, else classify latest via `home_status` shared precedence, CLEAN+stale→WARNING) + total `to_run_row`/`to_run_rows`. NEVER reads raw `error`/`ANOMALY:` (privacy). IA-6.
- `src/ui_flet/components.py` — View glue (coverage-omitted): design-system primitives from `tokens` + `verdict_visuals` — `card(gradient=…)`/`hero_gradient`, `primary_button`/`secondary_button`/`text_button`, `metric_tile`, `run_table(rows)` (first `ft.DataTable`: table-wide conditional myBlueprint+ columns, text-first status + AA-safe tint, ✓/✗/— SFTP, no Error column), `FileChip`, `ErrorCard`, `HealthVerdictBanner`, `build_design_demo()`.
- `src/ui_flet/nav_rail.py` — View glue (coverage-omitted): `build_nav(*, ordered, selected_id, on_select, on_exit)` — flat state-aware `ft.NavigationRail` (brand mark `leading` + one destination per `ordered` entry + a persistent decouple-the-sync reassurance line above Exit in `trailing`), prominent group first. Owns NO lifecycle/config; selection by `dest.id`, native highlight. Rail extraction (0018) + IA-2 reassurance line.
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds themed window + `dict[id -> factory]` + rail via `nav_rail.build_nav` (id-keyed `select_by_id`); swaps `setup`/`home`/`convert`/`run_history`/`help` placeholders (`help` precedes the `DISTRICTSYNC_UI_DEMO` override so the dev demo wins); owns content host + zero-orphan close + `_on_leave` seam reading `convert.is_write_in_flight()` (logs only, never blocks the close).
...
- `src/ui_flet/screens/home.py` — View glue (coverage-omitted): `build_home(page, *, app_config, on_navigate)` — three-way Home dashboard. Branch (a) `nav.needs_setup`→reuses `build_onboarding`; (b)/(c) read `run_log.read_run_records`+`derive_home_status`, render verdict-first (`HealthVerdictBanner` + fix-path `primary_button` + `components.metric_tile` tiles from `HomeMetrics`). Sync read; never-crash `ErrorCard` wrapper. IA-3b.
- `src/ui_flet/screens/convert.py` — View glue (coverage-omitted): `build_convert(page)` manual convert (`PickerField`+`FileChip`s → `JobRunner.run` → verdict/tiles/quality; SFTP pre-flight `page.show_dialog` → exit-3 `BUILT_NOT_DELIVERED`) + `convert_job(config,input_dir,*,anomaly_ack,sftp_requested)` parity adapter off-thread (`run_transform`→`compute_anomalies`→ack-gated `save_all`→tight `upload_csvs` catch) + `is_write_in_flight()`. IA-5a/b.
