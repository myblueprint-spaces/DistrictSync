...
- `src/ui_flet/home_status.py` — COUNTED pure Home status-derivation (no flet; the trust core): frozen `FixAction`/`HomeMetrics`/`HomeStatus` + `STALE_AFTER_HOURS=36` + `is_stale` + `derive_home_status(records, app_config, *, now=None)` — ordered first-match rules (None→unavailable, []→empty, failed-ETL/SFTP→FAILED, anomaly/data-errors/stale→WARNING, else HEALTHY+metrics). TOTAL; reads FLAT count keys; NEVER surfaces `error` (privacy). IA-3a.
...
- `src/ui_flet/components.py` — View glue (coverage-omitted): design-system primitives from `tokens` + `verdict_visuals` — `card(gradient=…)`/`hero_gradient`, `primary_button`/`secondary_button`/`text_button`, `metric_tile` (shared Home+Convert), `FileChip(filename, *, present=True)` (amber `present=False` = missing cue), `ErrorCard`, `HealthVerdictBanner`, dev-only `build_design_demo()`.
...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds themed window + `dict[id -> factory]` + rail via `nav_rail.build_nav` (id-keyed `select_by_id`); swaps `setup`/`home`/`convert` placeholders; owns content host + zero-orphan close + `_on_leave` seam reading `convert.is_write_in_flight()` (IA-5b write-guard — logs only, NEVER blocks the atomic close).
