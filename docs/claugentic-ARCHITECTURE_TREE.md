...
- `src/ui_flet/screens/setup.py` — View glue (coverage-omitted): `build_setup(page)` — Setup folders surface (two `PickerField`s + SIS/district `ft.Dropdown` from `loader.available_configs()` with names via `humanize.friendly_district_name`, DRY single-sourced); Save (`primary_button` + `disabled_bgcolor`) structurally disabled until `setup_state(...).can_save`, persists input/output/SIS → `is_complete()`; mounted via `shell.build_screens`.
