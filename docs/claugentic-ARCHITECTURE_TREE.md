...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds the themed window + flat `NavigationRail` + a `dict[id -> placeholder factory]` of calm, branded placeholders (cards/buttons via the components layer); zero-orphan close (`prevent_close=False` + CLOSE → `window.destroy()`, `on_disconnect` → `os._exit`) with a documented `_on_leave` seam (IA-2/IA-5 attach); `DISTRICTSYNC_UI_DEMO` routes Help to the design demo.
