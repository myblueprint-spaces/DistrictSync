...
- `src/ui_flet/shell.py` — View glue (coverage-omitted): `main(page)` builds the themed window + a `dict[id -> placeholder factory]` + delegates the rail to `nav_rail.build_nav` (id-keyed `select_by_id`); swaps `setup`→folders and, when unconfigured (`not (is_complete() and schedule_registered)`), `home`→`build_onboarding` (IA-2); owns content host + zero-orphan close + no-op `_on_leave` seam; `DISTRICTSYNC_UI_DEMO` → demo.
