...
- `src/utils/paths.py` — Path-resolution single source: bundle helpers (`bundle_root`, `app_icon_path`, …), `user_data_dir()` (call-time app-data seam; step 0 = `DISTRICTSYNC_DATA_DIR`, wins outright/no fallback — the only frozen-exe redirect — then `platformdirs`, then legacy `~/.districtsync`), `migrate_legacy_data_dir()` (stage-then-promote + `MOVED.txt`; no-op under the override), `user_mappings_dir`/`user_log_file`/`user_history_db`.
