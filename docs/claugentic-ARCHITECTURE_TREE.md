...
- `src/etl/transformers/enrollments.py` — `EnrollmentTransformer`: asserts `context.class_artifacts` (fail-loud when Classes has not run); homeroom / subject+blended / co-teacher builders each RETURN a frame (or None), concatenated in fixed legacy order; student rows filtered to `active_student_ids` (zero-orphan); dedup on (Class ID, User ID, Role).
- `src/etl/transformers/blended.py` — `BlendedClassDetector` (plain service, LSP fix): identifies same-teacher/same-time-slot sections with 2+ grade levels; RETURNS `BlendedDetection` (class_map/metadata/teacher_map) for ClassTransformer to publish via `ClassArtifacts` — never mutates context; one-pass MT-ID→(position, teacher) index; falls back to deduplicated schedule when ClassInfo lacks required columns.
