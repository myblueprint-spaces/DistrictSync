...
- `src/etl/loader.py` — `DataLoader`: writes DataFrames as UTF-8 CSVs with YAML field ordering — BOM (`utf-8-sig`) for Excel, except `_NO_BOM_ENTITIES` (`StudentAttendance`) written plain UTF-8 for SpacesEDU's BOM-strict parser; `save_all()` stages to `.tmp_<timestamp>/` and atomically commits so a mid-write failure leaves existing output intact.
