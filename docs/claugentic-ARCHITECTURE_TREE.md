...
- `src/etl/pipeline.py` — Core ETL orchestration (`run_pipeline`, `run_transform`, `extract_required_files`, `compute_anomalies`): coordinates Extractor → Transformer → Loader, anomaly detection, diff, quality report, structured JSON run-log, and optional SFTP upload; `run_transform(raw_data, mappings, global_config) -> TransformOutputs` (NamedTuple of `outputs` + `field_orders`) is the shared transform-orchestration seam — school-year determination + `enabled_entities`/`entity_order` filtering + per-entity loop + field-order collection — called by `run_pipeline`; `compute_anomalies(outputs, output_dir) -> list[str]` is the single-sourced row-count-drop compute (plain `ANOMALY_THRESHOLD`-driven warning strings, no logging) consumed by both the CLI (`_check_anomalies` thin wrapper adds the `ANOMALY:` prefix + `logger.warning`) and the Convert page (`st.warning`); returns `PipelineResult` dataclass (`entity_counts`, `sftp_attempted`, `sftp_ok`, `anomalies`); logs ERROR and propagates failure so `main` can exit 3 on SFTP failure; importable by UI and tests.
...
- `src/etl/transformers/base.py` — `BaseTransformer` ABC with utilities shared by all entity transformers: CEDS grade mapping, `ALLOWED_TRANSFORMS` security allowlist, `apply_field_map()`, `assign_class_ids()`, excluded-course-code filters, the config-driven active-student predicate (`resolve_active_config()` / `is_active_mask()` / `past_withdraw_date()`), and `filter_to_active()` (zero-orphan roster filter shared by Classes + Enrollments).
- `src/etl/transformers/context.py` — `TransformContext` dataclass: mutable shared state for one pipeline run (school year, academic dates, raw data frames, `active_student_ids` roster published by Students, homeroom/blended maps) passed between entity transformers.
