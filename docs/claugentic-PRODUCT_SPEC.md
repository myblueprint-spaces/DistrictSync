...
- **States** — **empty** (no folder picked; "Choose your district" placeholder; Convert disabled until a district and output folder are set), **loading** (a spinner while the worker builds), **error** (a category card worded from the failure's TYPE, the same words Home and Run History use — only a missing or unreadable input names the input folder; the raw exception discarded; it says nothing new was saved rather than promising the old files are untouched; a fresh install leads with a routed "Finish setup first" card). Anomalies (>20% drops) gate delivery behind an explicit acknowledgment.
