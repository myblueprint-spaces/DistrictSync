...
| `check_row(label, *, value, on_toggle)` | a tick-list row (entities, grades), and a **disclosure** that reveals a block of copy | `ft.Checkbox` with token `active_color`, `on_change` adapted to `on_toggle(bool)`; the ONE checkbox spelling (0044 S3). Used for Setup's gMSA disclosure (0049 S-4) in preference to the raw `ft.Switch` the seasonal-window section next to it predates the factory with — the two co-exist in that file, and this rule is the one that decides new work |
