...
- There is a fourth option if your directory can supply a **managed service account (gMSA)**: nothing is stored on the computer, so this policy does not block it. See *[Managed service accounts (gMSA) — what your IT team needs to do](managed-service-accounts.md)*. It is **available but untested** — we have no domain to test it against — so treat it as something to try with your IT team rather than a known fix.
...
- This path has **not been tested against a live domain**. If you get here and your directory administrator confirms all three prerequisites are in place, that is a result we want: the Help page in DistrictSync has our support contact, and the Windows code shown on screen is what we need.
