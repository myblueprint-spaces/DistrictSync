...
- **P5 — A missing column is handled by what it GUARDS: (a) PII scope and (b) join key fail CLOSED, typed, never a raw `KeyError` or partial result; (c) contract field excludes and counts; (d) optional field blanks, warns once, notes; (e) safety heuristic fails open only toward surplus, always recorded.** _(Plan 0053 · `failure-policy.md` §5 + its site catalogue.)_ **Planned (S1, S6, S9, S10, S11 per site).** Four sites already conform (§5 #7, #20–#22; #22 excluding its evidence reader #22a).

- **P6 — Every boundary-crossing ETL failure is a typed `EtlError` with a bounded category; classification is by `isinstance` only, never message text; every closed-enum member maps to copy, a verdict and a doc row.** _(Plan 0053 · `failure-policy.md` §6.)_ **Planned (S1 taxonomy, S3 copy).** Today the classifier tests `"No usable required input" in str(exc)` (`pipeline.py:759`).
