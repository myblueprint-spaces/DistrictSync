...
- **P4 — Never substitute a FAILED or EMPTY entity (no last-good CSV, no unfiltered rows, no partial frame, no filter-less mapping); its previous CSV leaves the delivery glob; its absence is reported every run it persists.** _(Plan 0053 · `failure-policy.md` §4.)_ **Enforced** for the archive/manifest half, and since S3 for the READER of the every-run report (a FAILED outcome in a completed run's own record is PARTIAL / WARNING on Home, Run History and Convert); **enforced since S4** for the PRODUCER (the bulkhead leaves a FAILED entity out whole, and its record carries the FAILED outcome every night — the Unity plain-report contract fixture and the two-night test); **enforced since S10** for the partial frame (the homeroom enrollments shipped after a caught `KeyError` are gone).

- **P5 — A missing column is handled by what it GUARDS: (a) PII scope and (b) join key fail CLOSED, typed, never a raw `KeyError` or partial result; (c) contract field excludes and counts; (d) optional field blanks, warns once, notes; (e) safety heuristic fails open only toward surplus, always recorded.** _(Plan 0053 · `failure-policy.md` §5 + its site catalogue.)_ **Enforced since S10 for classes (a) and (b)** — every guard is ONE call, `columns.require_columns` (the §5 `require-columns` table, pinned both ways to the call sites by AST, each call tagged `# failure-policy:` directly above it), naming every missing column in config spelling + a column COUNT + `header_looks_like_data`, never a header; a guard runs once its source is present and non-empty and the column is about to be read. (b)'s reads whose direction is still open — #19, #33, #34, #34a — are RECORDED since S11 (one WARNING + a note each), never silent. Since S6 class (d)'s "warns once" holds for an ABSENT mapped column (one WARNING per entity + `missing_mapped` on the outcome — the note is S11), and since S10 for the homeroom teacher name (#35 — blank + one WARNING, never fatal) and — owner ruling 2026-09-25 — the ClassInformation co-teacher columns (#15 — rows left out + one WARNING + the first outcome note, `coteacher_source_unusable`; ClassInformation's school stays (b)). **Enforced since S11** for (c)–(e)'s recorded notes: every such site logs at most one line and records one `OutcomeNote` per entity per run, and a FULL bijection pins every `# failure-policy:` tag under `src/` to the §5 catalogue (`tests/test_failure_policy_parity.py`). What must stay true: a (b) fault never ships part of an entity.

- **P6 — Every boundary-crossing ETL failure is a typed `EtlError` with a bounded category; classification is by `isinstance` only, never message text; every closed-enum member maps to copy, a verdict and a doc row.** _(Plan 0053 · `failure-policy.md` §6.)_ **Enforced** for the taxonomy and type-only classification since S1 (`src/etl/errors.py`; `tests/test_etl_errors.py` AST-pins the classifier); **enforced** since S3 for copy — `src/ui_flet/failure_copy.py` is TOTAL over every category (NONE raises), every valid (kind, reason) and every kind, pinned from the enums and against `failure-policy.md` §6; **enforced since S10** for the §5 missing-column sites (no catalogued fault reaches the pipeline as a raw `KeyError`).

- **P7 — Level-triggered: every configured entity has exactly one outcome per run, and a FAILED outcome in a successful run is PARTIAL / WARNING every run it persists, derived from that run's own record — never from a baseline the run archives.** _(Plan 0053 · `failure-policy.md` §7.)_ **Enforced since S2 for the first half** — every configured entity has exactly one outcome per run in the record's `entity_outcomes` (`OutcomeLedger.complete()` refuses a gap; `tests/test_pipeline_run_store.py`). **Enforced since S3 for the reader of the PARTIAL/WARNING half** — `home_status.LatestReason.PARTIAL` (below FAILED_DELIVERY, above ANOMALY), derived from the record's own `entity_outcomes` (a delivery-only record inherits its build's through the REQUIRED `prior_build`), identical on Home, Run History and Convert. **Enforced since S4 for the producer** — the bulkhead's FAILED outcome rides a completed run's record, so the warning persists every night the entity stays out, while the vanished-entity anomaly fires only the first night (measured: `tests/test_pipeline_entity_isolation.py`'s two-night test). **Extended by S8 (owner decision D5)** to an entity that built NOTHING: `failure_copy.OUTCOME_TIER(entity, kind, reason)` rates EMPTY/`no_rows_after_transform` and EMPTY/`missing_source_column` WARNING on every entity, and EMPTY/`source_files_empty`/`no_source_files_declared` WARNING on every entity outside `outcomes.MAY_BE_EMPTY` (today `{StudentAttendance}`); `failure_copy.warning_outcomes` is the ONE PARTIAL predicate for Home, Run History and Convert. **Extended by S10 (owner ruling 2026-09-25)** to a BUILT entity carrying a WARNING-tier note (`failure_copy.NOTE_TIER`; today only `coteacher_source_unusable`): `failure_copy.outcome_tier` takes the WHOLE outcome, so `warning_outcomes` stays the one predicate and a noted build is PARTIAL every night it persists (`tests/test_coteacher_standing_warning.py`, two nights end to end) — except on a night with an anomaly, when a PARTIAL made only of notes (every warning outcome BUILT: the file was delivered) ranks BELOW ANOMALY, so a standing note never hides a one-night drop. What must stay true: an unlisted entity warns (never quieter by omission), a district that warns is fixed by a config change and never by widening `MAY_BE_EMPTY` without a DECISIONS entry, and BUILT never warns (`tests/test_standing_empty_warning.py`, two nights end to end).
...
- **P9 — Bounded surfacing: records, banners and cards carry closed-set codes humanised through total copy tables — never `str(e)`, cell values, paths or OBSERVED header text; config-DECLARED labels only, membership-validated and sanitised (D4).** _(Plan 0053, D4 decided 2026-09-23 · `failure-policy.md` §8.)_ **Enforced** for the category-only store and fixed copy — since S3 through the total tables in `src/ui_flet/failure_copy.py`, whose only variable text is an authored entity phrase and counts (sentinel-swept); **enforced** since S1 for the four sites that dumped observed headers into exception/log text (count only; sentinel-pinned); **enforced** since S7 for labels: `outcomes.safe_label` (membership in the resolved config's own vocabulary + printable, ≤ 120, no newline, never email-/path-shaped) is the one check and `outcomes.apply_labels` the one producer; copy interpolates nothing else.

- **P10 — One column resolver (`columns.resolve_source_column`) with one shape policy built on `models.classify_field`; presence via `columns.require_columns`; new shared behaviour goes in composed modules, not `BaseTransformer` methods.** _(Plan 0053 · `failure-policy.md` §9.)_ **Enforced since S9 for the resolver:** every transformer source-column read with a default goes through `columns.resolve_source_column` — since S11 including `BaseTransformer.resolve_active_config`'s EnrollStatus `status_column`/`withdraw_date_column` (the alias detection and the presence check stay there, recorded as §5 #17/#18) — (one shape policy on `models.ensure_field_mapping`; the three old resolvers and every inline `.get(...).lower()` deleted; the Students mapping reaches the homeroom path through `context.entity_mappings`), AST-pinned in `tests/test_column_resolution_pin.py` with doctored twins. Composed-module rule review-held (S9 deleted a `BaseTransformer` method, added none); its fitness pin is S13a. **Enforced since S10:** presence via `columns.require_columns`, the one fail-closed check (same pandas-free module).

- **P11 — Source observation never enforces: it attributes missing mapped columns per entity over that entity's OWN files, never raises, never gates, and keeps preflight's soundness rule.** _(Plan 0053 · `failure-policy.md` §10.)_ **Enforced since S6.** `pipeline.observe_source_columns` — called by BOTH entry points after the input is read and before the transform (AST-pinned, `tests/test_pipeline_run_store.py`) — derives the misses with `preflight.missing_columns_by_entity` (own files per `preflight.OBSERVATION_SCOPE`, registry-complete; `source_columns` excluded; no claim unless every own file was observed with headers), logs one WARNING per entity and carries them on the outcome as `missing_mapped`; its broad handlers — one around the derivation, one per entity so a refused note costs only that entity — log at DEBUG, and the raise-isolation twin proves a raising observation leaves delivered bytes, status, category and counts identical. It changes exactly one reason (EMPTY/`no_rows_after_transform` → EMPTY/`missing_source_column`, `outcomes.apply_observation`) and never a FAILED or BUILT one. **Guards are enforced once, where the column is USED (§5) — never by the observation.** The creator's file-agnostic `preflight_report` is unchanged.

- **P12 — Additive record evolution: new facts are new JSON keys; no `runs` DDL, CHECK or `user_version` change until the ROADMAP migration-ladder item is fixed; readers are TOTAL; an unknown kind/reason from a newer build reads as FAILED/TRANSFORM_ERROR.** _(Plan 0053 · `failure-policy.md` §7.)_ **Enforced** for no-DDL and total readers, including the outcome reader since S2 (`outcomes.outcomes_from_record`: never raises; unknown kind/reason → FAILED/`transform_error`; since S6 an unusable `missing_mapped` reads as `()` and keeps its entry). S6's `missing_mapped` is itself additive: written only when non-empty, so every other entry is byte-identical — and so is S10's `notes` (`{note: count}`), whose reader drops an unusable pair and an UNKNOWN note code (a newer build's) rather than read it as a warning (DECISIONS 2026-09-25).
...
- **P14 — Docs follow code and are pinned: every rule row carries `ENFORCED` or `PLANNED (0053 Sn)`; a slice flips its own rows in the same change; every table mirroring a code constant has a parity test with a non-vacuity assertion and a doctored-doc twin.** _(Plan 0053 · `failure-policy.md` §0.)_ **Enforced since S2 for the two code-mirroring tables** (§3 criticality, §6 vocabularies — `tests/test_failure_policy_parity.py`, with non-vacuity and doctored-doc twins); the Status column and every other row are still held true by review. Since S4 the parity test also pins the partner FAQ (criticality bullets ↔ §3, pending clause ↔ `Q5-status`); since S8 the §7 `may-be-empty`/`outcome-tier` tables; since S10 the §5 `require-columns` table (by AST, both directions) and the §7 `note-tier` table; since S11 the §5 site tags (a full bijection with the catalogue, class-checked, every `record_note` call tagged, every note produced).

- **P15 — Config typos are loud and origin-keyed: a bundled config RAISES at load; a user-dir overlay WARNS at run and is REFUSED at authoring; `Field*` leaf models forbid extras everywhere; the root stays `extra="ignore"`.** _(Plan 0053 · `failure-policy.md` §6 (config-typo row) · `src/config/loader.py` (`unknown_config_keys`, `_UNKNOWN_KEY_POLICY_BY_ORIGIN`, `UnknownConfigKeyError`) + `src/config/models.py` (`ConfiguredField` forbids, `classify_field`).)_ **Enforced since S12**, pinned by `tests/test_config_unknown_keys.py` (each negative with a positive twin; every bundled config walks to zero findings). Same direction as the `district_domains` floor above — never invert: do not make a user-dir unknown `global_config`/entity key or `source_columns` role raise (a stray key must never stop a nightly), do not make a bundled one warn, and do not let `validate_overlay` merely warn. The `source_columns` role vocabulary is each transformer's `SOURCE_COLUMN_ROLES`, derived from the constants its read sites resolve — never a list restated beside the walker. The one deliberate all-origins refusal is a key inside a field mapping (D11: `transfrom:` is wrong output), so a hand-placed user-dir config carrying one stops its nightly until fixed.
