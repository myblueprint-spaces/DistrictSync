...
2. **`python scripts/claugentic-check_architecture_tree.py`** green (the one mechanically-enforced harness gate — file-index presence, staleness, **glob-drift detection**, and a per-entry **form budget**). The form budget (`MAX_ENTRY_CHARS`, default 450) is a sub-check **of this same gate** — it inherits the gate's **hook-enforcement** (it's the index-well-formedness invariant the gate already owns, not a new gate); the tree check stays the **one** hook-wired gate.
   ...
   - *(Adopter note: this is the harness's OWN self-test gate, hook-wired by `init` only when the tree-gate is on. A keep-your-own-tree repo runs gate-off — substitute your project's own structure check.)*
3. **`python scripts/check_versions_synced.py`** green — enforces `plugin.json` ↔ `marketplace.json` version equality (`plugin.json` is the source of truth). A **run-gate** executed in this gate suite (like `pytest`), not hook-wired; scope is the two manifest versions only. *(Adopter note: this is the harness's OWN self-test — it checks the *plugin's* two manifests and is irrelevant to an adopter; substitute your project's equivalent.)*
4. **`python scripts/check_doc_budgets.py`** green — flags when a managed ledger (CLAUDE.md / DECISIONS / ROADMAP) outgrows its TOTAL byte budget and names the remediation (a compaction pass; merge superseded entries → git history). A **CI run-gate in the same register as `check_versions_synced.py`** — **mechanical-when-run** (plain messages, no model judgement; running it is discipline), **NOT hook-wired** (the tree check above stays the one hook-enforced gate). It **flags, never edits** — the compaction pass itself is model-upheld (below).
5. **The project's lint / type-check / security gates** green.

**Reviewer sign-offs** (model judgment, not a mechanical gate):
6. **In-scope `docs/claugentic-standards/` dimensions pass** the `architect-reviewer` audit (incl. SOLID) — solo, or synthesized from `lens-reviewer`s + `yagni-sentinel` — for what this slice touches; `/simplify` + `/code-review` run.
7. **Runtime QA (dial-gated)** — when the slice's spec carries acceptance criteria (the `docs/claugentic-PRODUCT_SPEC.md` schema) and the repo records a run-the-app command, and the Verify dial has fanned out: the QA workflow (`engine/qa.js`) attempts each criterion's flow in the running app and tags the outcome (screenshots on failure); findings join the Verify synthesis. The script mechanically sequences the flow attempts and never silently skips (a QA run that could not boot the app reports exactly that); invoking it, and the flow observations themselves, are model-upheld.

**Plus:** acceptance criteria met (the spec's checklist) **+ no new tech debt.**

**The model-upheld curation halves (flag-and-suggest only — no automation this pass):** the two form/size gates above are deterministic *detectors*; the *fix* stays model-upheld. (1) **Scoped single-line regen on a form-flag** — when the tree gate flags an over-budget entry, rewrite **that one entry** to a budgeted line (role + key collaborators + pointer), relocating genuinely-useful mechanism detail to the file's own header (read-on-open, not read-every-session); don't touch the rest of the index. (2) **Bounded compaction pass on a budget-trip** — when `check_doc_budgets.py` flags a ledger, run a **bounded** curation pass: merge superseded/duplicated entries into git history (the archive), keep the standing record; never mass-rewrite. Both are **flag-and-suggest** — the gate names the action, the agent performs it under review; **no scoped-regen / compaction automation is built this pass** (YAGNI — earn it on a proven second trigger).
