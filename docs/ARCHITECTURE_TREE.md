...
- `src/etl/transformers/students.py` — `StudentTransformer`: filters to active students (Active only by default; `active_values` configurable) via the shared `BaseTransformer` active predicate (config-resolved status column / withdraw-date hard override), logging a per-status drop breakdown; publishes the filtered roster to `context.active_student_ids` for downstream zero-orphan filtering; generates template emails; normalises ISO dates for Date of Birth.
