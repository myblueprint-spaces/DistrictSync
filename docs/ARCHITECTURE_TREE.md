...
- `config/mappings/sd54myedbc_mapping.yaml` — SD54 Bulkley Valley override (`_base: myedbc`): lowercase/non-default source filenames (`studentschedule.txt`, `courseinformation.txt`, `staffinformation.txt`, `classinformationenhanced.txt`), `{legal surname}.{usual first name}@sd54.bc.ca` email, `excluded_course_codes` for ATT--AM/PM/Daily; field maps inherit from base unchanged (academic dates auto-derive from the schedule's School Year column).
