...
- `src/etl/transformers/base.py` — `BaseTransformer` ABC: CEDS grade mapping, `ALLOWED_TRANSFORMS` security allowlist, `apply_field_map()`, `assign_class_ids()`, `filter_excluded_course_codes()`, `filter_excluded_course_code_patterns()`, `clean_course_code_flavor()`, the config-driven active-student predicate (`resolve_active_config()` / `compute_enroll_status()` / `is_active_mask()` / `past_withdraw_date()`), `filter_to_active()` (zero-orphan roster filter shared by Classes + Enrollments), and other utilities shared by all entity transformers.
...
- `src/etl/transformers/students.py` — `StudentTransformer`: filters to active students (Active + PreReg by default; `active_values` configurable) via the shared `BaseTransformer` active predicate (config-resolved status column wins; withdraw-date is a fallback for rows with no status value), logging a per-status drop breakdown; publishes the filtered roster to `context.active_student_ids` for downstream zero-orphan filtering; generates template emails; normalises ISO dates for Date of Birth.
...
- `config/mappings/sd54myedbc_mapping.yaml` — SD54 Bulkley Valley override (`_base: myedbc`): lowercase source file names (`studentschedule.txt`, `courseinformation.txt`, `staffinformation.txt`, `classinformationenhanced.txt`), non-Enhanced `staffinformation.txt` for Staff, `EmergencyContactInformationEnhanced.txt` for Family, `{legal surname}.{usual first name}@sd54.bc.ca` email, `excluded_course_codes` for ATT--AM/PM/Daily; academic dates auto-derive from the schedule School Year (end-year convention).
