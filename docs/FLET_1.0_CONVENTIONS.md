...
- **CONTRACT (confirmed against `flet==0.85.3` in PLAT-1; the `JobRunner` CODE is deferred to IA-5 — its first real `run_pipeline` caller, per the program's "promote on 2nd use" rule — but the contract is fixed now):**
  - **`Page` exposes both `page.run_thread(handler, *args)` (run a blocking fn off the UI thread) and `page.run_task(coro)` (schedule a coroutine on the loop).** The worker does its blocking work on the thread, then marshals each UI update back via `page.run_task(...)` (or by mutating controls only inside a coroutine the loop owns) and calls `page.update()` there — never from the worker thread directly.
    ...
    ```
  - **`SystemExit` vs `Exception` `on_error` ASYMMETRY (load-bearing):** `run_pipeline` calls **`sys.exit(1)`** on bad input (input dir missing, config `FileNotFoundError`, config `ValueError`). Since plan 0034 Slice 4 these paths record a failed run to BOTH sinks **before** exiting (`pipeline._record_early_failure` — `error_category` `no_input`/`config`), so Run History records them (best-effort, like every store write); a caught `Exception` also writes a "failed" record via the generic failure sink. The worker MUST still catch `SystemExit` (it is NOT an `Exception` subclass — a bare `except Exception` lets it propagate and kill the thread silently) and surface it to the user.
...
- **Measured size (PLAT-3, LANDED):** the **real `DistrictSync` windowed onefile ≈ 190 MB** (`189,638,864` bytes — full flavor, Windows, local Win11 build with the full `src.*`+flet hidden-import set). Much larger than the ~58 MB flet-only spike because it bundles pandas/pydantic/paramiko/keyring + the ETL core; the exe is `--onefile` (compresses the ~96 MB uncompressed client) and `--exclude-module streamlit`/`src.ui` is belt-and-braces (both removed at CUT-1). That number is the retired **`full`** flavor; the 2026-07-28 `light` flip is **expected** to bring it down (the embedded client is ~99.8 MB uncompressed of it) — expected, not measured here. The workflow echoes the measured size in **bytes** into the job's step summary each build — read the current per-OS numbers there and in the DECISIONS reversal entry, not from this line, and treat "does not grow" as the budget so an unexercised transitive can't quietly balloon it.
- **macOS `flet pack` bundling — VERIFIED in CI (2026-07-28).** PLAT-0 could only confirm embedding on **Windows**, and the upstream 0.85.0 "bundle the client into `flet pack`" fix names **Windows + Linux** only, so the macOS path (`.tar.gz`/`.app` + `FLET_VIEW_PATH`) was tracked as an open risk. It is now exercised on a `macos-latest` runner every pack: the build-time `--assert-embed` finds the client archive under `flet_desktop/app/`, and the runtime move-aside launch repopulates `~/.flet/client` with `FLET_CLIENT_URL` unreachable. What is still NOT asserted on macOS/Linux: the window itself — `_phase_close` waits for a top-level window and posts `WM_CLOSE` **only under Windows**, so the close axis is INFO-only there (and no macOS check asserts a window ever painted).
...
- **Cache key MUST include the OS + flavor.** The client is per-OS: Windows `flet-windows.zip`, macOS `flet-macos.tar.gz`, **Linux `flet-linux-{distro}[-light]-{arch}.tar.gz`** (only the Linux name encodes distro/flavor/arch — the Win/mac archive names are flavor-INDEPENDENT on 0.85.3, verified in `flet_cli/commands/pack.py`; the cache DIR `flet-desktop-{flavor}-{ver}` is flavor-keyed on every OS). On Linux, **pin the runner image** (e.g. `ubuntu-22.04`, not floating `ubuntu-latest`) so the distro-keyed filename doesn't drift. **We ship `light` on all three OSes** (2026-07-28, D-0037-1 — the app plays no audio/video); `flet_desktop`'s own default is `light` on Linux and `full` elsewhere, so Windows/macOS must set `FLET_DESKTOP_FLAVOR=light` EXPLICITLY (the CI matrix and the `Makefile` both do).
...
- **The cache key MUST encode the flavor, not hardcode one.** Set `FLET_DESKTOP_FLAVOR` explicitly so warm-up, cache key, and `flet pack` provably agree — **`light` on all three OSes** since 2026-07-28 (PLAT-0b proved `light` is enough on Linux; the D-0037-1 flip PR's own 3-OS run is the Windows/macOS evidence, and the pre-tag QA walk is the rendered-content gate). A warm-up whose flavor disagrees with the pack silently caches a client the build then does not use. Confirm `pyproject.toml` has **no conflicting `[tool.flet]` desktop flavor** (it doesn't, as of PLAT-3).
- **Reference YAML (per-OS matrix step, before `flet pack`) — LANDED in `.github/workflows/flet-pack.yml` (PLAT-3):**
  ...
  # ... matrix sets FLET_DESKTOP_FLAVOR per-OS (light on win/mac/linux since 2026-07-28)
  steps:
