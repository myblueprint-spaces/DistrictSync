...
| `unitychristianmyedbc` | Students, Staff, Classes, Enrollments | Students, Staff, Classes, Enrollments | Unity Christian School, Chilliwack (2026-09-01) — an independent K-12 school, and the first config built from a REAL first GDE drop rather than assumed naming (filenames and columns are canonical, so it carries no `source_files`/column overrides). **`Family` is deliberately NOT enabled:** the school's `EmergencyContactInformation.txt` has no email column at all, so the entity could only emit blank-`Email` rows — the field a SpacesEDU family account is keyed on — for ~1,700 non-guardian emergency contacts. `homeroom_grades` runs through 08 because the schedule export covers grades 9-12 only; student emails are generated as `{student number}@unitychristian.ca`. |
