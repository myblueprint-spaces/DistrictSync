...
| `sd75myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier on base defaults. Staff domain is `mpsd.ca`. A pre-existing USER-dir `sd75myedbc` (the retired v2.x Mapping Editor wrote one) shadows this bundled config — the loader logs the shadow. Standard file naming assumed. |
