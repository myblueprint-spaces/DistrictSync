...
| Rostering bundle | The five **rostering** CSVs — `Students`, `Staff`, `Family`, `Classes`, `Enrollments` — ship inside one zip named `districtsync_<district>_YYYY-MM-DD.zip` (e.g. `districtsync_sd40_2026-04-10.zip`); `districtsync_YYYY-MM-DD.zip` when no district is supplied. Single source: `sftp/uploader.build_zip_name`. **Changed in contract 2.0.0** — the two course feeds used to ride this bundle. **Not every night carries all five:** an entity that builds no rows is skipped (never written header-only), and since contract 2.7.0 `Family.csv` is also absent on a night its export fails — the other four still ship, and the run is flagged PARTIAL (plan 0053 S4). What the importer does with family links an earlier delivery created is pending confirmation (Q5). | pending owner confirmation | emitted | GUARANTEED |
