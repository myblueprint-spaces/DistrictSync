...
| **emitted_by** | DistrictSync — `2.5.0`, emitted since the release carrying the blended class-name change below (unreleased at the time of writing). `2.4.0` was emitted since v3.17.0 (the release that carried the `Staff.csv` and `StudentAttendance.csv` row-set changes). None of v3.18.1, v3.19.0 or v3.20.0 changes a contract row: SD51's period-band filename fix is a district `source_files` correction, SD83's `Prefix`-sourced roles are per-district config, and v3.20.0's `blended_classes` opt-out is per-district config whose only consumer (SD51) emits byte-identical files today — none is a rule change, following the 2026-09-10 SD60 precedent. The `Staff.csv` Role row's DESCRIPTION gained the second derivation mechanism (`normalize_staff_role`), but the emitted vocabulary is unmoved. **Note:** the binaries published as v3.18.1 were built from v3.18.0's commit, so the newest tagged build actually emitting `2.4.0` behaviour for SD51 is v3.19.0. |
...
| 2.3.0 | 2026-09-09 | **Rows removed from `Staff.csv`.** A staff record the source export marks as departed — or leaves blank — is no longer emitted: MyEd BC's staff export is unfiltered and carries former employees alongside current ones, so shipping them created active SpacesEDU accounts for people who had left the district. The rule engages on the DATA (the resolved status column, when present, matching a recognised Active/Inactive vocabulary), not on per-district config, so it applies to every district without an opt-in. It fails OPEN — ships every row — when the vocabulary is unrecognised or when the filter would empty the file, so `Staff.csv` is NOT guaranteed free of departed staff, only cleaned wherever the export lets us recognise them. No column, order, filename or encoding changed, and no owner-**confirmed** row is invalidated (every `Staff.csv` row is `pending owner confirmation`) — MINOR under the row-set rule below, no importer re-confirmation needed. |
...
>
> **Row-VALUE changes** (2026-09-16). A change to the **value a column carries**, with the row set and the column shape both unmoved, is **MINOR** plus a changelog row when it invalidates no owner-**CONFIRMED** row; it is **MAJOR**, and requires importer re-confirmation before merge, when it does. Same shape as the row-set tier and written for the same reason: the two rules above cover column shape and row membership, and said nothing about the values inside a row. `2.1.0` is the precedent this codifies — it changed values in `StudentCourses.csv` and graded itself "MINOR under the row-set rule", stretching a rule that did not cover it. A value change that would alter how a row is **matched or keyed** on ingest is never merely a value change: treat it as MAJOR.
...
**…unless the district scopes which grades it sends.** `global_config.student_rostering_grades` (opt-in; **no shipped district sets it today**) names the complete set of CEDS grades whose students reach the output at all. When it is set, a student appears only if their grade is in that list **and** their status is not `Inactive` — the two rules are ANDed. Because every other student-bearing feed is filtered against this file's roster, the excluded grades also lose their `Family.csv` guardians, their `Classes.csv`/`Enrollments.csv` rows and their `StudentCourses.csv` transcripts. Class rostering cannot reach outside it (see `Classes.csv`), and a blank/unrecognised grade maps to `UG`, so a district wanting those students must list `"UG"`.
...
| 5 | Role | `teacher` or `administrator` — those two values are the entire vocabulary and are asserted per config. Derived one of two ways, per district: the default `map_role` transform reads the teaching-staff FLAG (`Y` → `teacher`, anything else → `administrator`), while `normalize_staff_role` reads a column that STATES the role and refuses any other value (SD83 repurposes MyEd BC's `Prefix` this way). | pending owner confirmation | emitted | GUARANTEED |
...
A district may additionally narrow the source rows with `row_filters` before mapping — SD83 keeps only rows whose `Prefix` states a real role, so a courtesy title in that repurposed column never becomes a SpacesEDU user. This is opt-in per district and complements the departed-staff rule above rather than replacing it; note it fails **closed** (a `row_filters` column missing from the extract raises), which is why employment status is decided by the data-keyed rule and not by a filter. Matching is case- and whitespace-insensitive but is exact set membership, never a substring test. Where the district's role column carries a value the filters did not already exclude, `normalize_staff_role` blanks THAT CELL and records a data error rather than defaulting to a role — the run still delivers.
...
**The blended composite** (since contract 2.5.0). A blended class has no single section letter to name it by, so it is composed as:

```
<Teacher last> <Course title / Course title …> (Block <slot>) (<Grades>) <Year>
```

Example: `Clark Carol Business 11 / PHE 9 (Block 1 2 1 2) (08/11) 2026`. `<Grades>` is the blend's grade range — the one grade signal a blended row carries, since its `Grade` column is deliberately blank. `<slot>` is the blend's `term/semester/day/period` values, joined by spaces in that order and **never labelled individually**: the components' meanings differ per district, so naming one "Day" would assert an order the export does not guarantee. Components a district omits are absent from the slot, and a district carrying none of the four gets no `(Block …)` segment at all. The teacher is read from the same configured column the subject composite uses; where a district's class-information export omits it, it is resolved from the schedule instead, so blended and subject names show the same fact.
...
| `sd10myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); moved OFF the mbp_core shape onto the full myBlueprint+ tier (2026-09-15) when the district asked for class/enrollment/family rostering — `Staff` came with them because `Enrollments` always emits teacher rows, which would otherwise reference people who never ship. Carries NO grade keys: the base K-7 `homeroom_grades` and an unset `class_rostering_grades` already give K-7 homerooms plus 8-12 timetable classes, and pinning a literal 8-12 list would strip classes from any student whose Grade converts outside it (blank → `UG`). Generated `{student number}@sd10.bc.ca` emails. Standard MyEd BC file naming assumed pending real GDE samples. |
| `sd27myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier scoped to grades 8-12 (`student_rostering_grades`, first shipped consumer — declares `version: '1.11'`); `homeroom_grades: []`, so every class comes from the timetable. Standard file naming assumed. |
| `sd38myedbc` | Students, CourseInfo, StudentCourses | Students, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); moved to the mbp_core shape (2026-09-08) — no SpacesEDU rostering entities, student scope widened to grades 7-12, cross-enrollment collapse (`home_school_column: "Home School Number"`) since its real export sends one demographic row per school for a cross-enrolled pupil, and `Email Address` re-sourced from `Student Email` (its real column name, not the base default `Student email address`). Standard file naming assumed. |
| `sd67myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier on base defaults. Standard file naming assumed. |
| `sd69myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier with `course_start_grade: 9` (9-12 course feeds). Standard file naming assumed. |
| `sd71myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier on base defaults. Standard file naming assumed. |
| `sd75myedbc` | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Students, Staff, Family, Classes, Enrollments, CourseInfo, StudentCourses | Phase-2 migration (2026-08-31); full myBlueprint+ tier on base defaults. Student `Email Address` is GENERATED as `{student number}@learn75.ca` (2026-09-04) — MyEd BC holds non-district addresses for some Mission students, so the demographic column is not used. Staff domains are `mpsd.ca` + `sd75.bc.ca`; `learn75.ca` is the STUDENT domain and is deliberately not among them. A pre-existing USER-dir `sd75myedbc` (the retired v2.x Mapping Editor wrote one) shadows this bundled config — the loader logs the shadow. Standard file naming assumed. |
| `unitychristianmyedbc` | Students, Staff, Family, Classes, Enrollments | Students, Staff, Family, Classes, Enrollments | Unity Christian School, Chilliwack (2026-09-01) — an independent K-12 school, and the first config built from a REAL first GDE drop rather than assumed naming (filenames and columns are canonical, so it carries no `source_files`/column overrides). **`Family` was OFF until the 2026-09-14 drop:** the school's first `EmergencyContactInformation.txt` had no email column at all, so the entity could only emit blank-`Email` rows — the field a SpacesEDU family account is keyed on — for ~1,700 non-guardian emergency contacts. That drop switched to the ENHANCED contact report under the SAME canonical filename (so still no `source_files` override), supplying both `Email Address` and `Parent Auth / Guardian`; Family is now enabled behind a `row_filters` on that flag, the same shape `sd60myedbc` uses, because the export is an emergency-contact list rather than a guardian list. `homeroom_grades` runs through 08 because the schedule export covers grades 9-12 only; student emails are generated as `{student number}@unitychristian.ca`. |
...
> **Q4 — a renamed class: does the live importer update the existing class in place, or create a new one?**
> DistrictSync has twice changed `Classes.csv` `Name` values for classes that already exist in SpacesEDU — subject names on 2026-07-20, blended names at contract 2.5.0 — each time on the understanding that the importer matches a class on `Class ID` (unchanged in both) and so treats a changed `Name` as a rename rather than a new class. That understanding has never been confirmed against the live importer. If it is wrong, a rename orphans the original class and its student work.

*The check:* import a `Classes.csv` whose `Name` differs from the previous import for one unchanged
`Class ID`; confirm the existing class is renamed and retains its enrollments and content, rather than a
second class appearing. **Worth settling before the next name change, not urgently after this one** — two
