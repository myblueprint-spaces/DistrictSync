...
| S2 | **DONE — integrated** | `claude/0053-s2-outcome-ledger` → `claude/0053-etl-failure-policy` | Outcome ledger + criticality + `entity_outcomes` record key; no behaviour change. 3-lens review: all APPROVE; 3 findings confirmed + fixed, 2 refuted. Local: 7,848 passed / 98 skipped, coverage 96.60%, all gates green. Real drops: CSVs byte-identical; Unity plain → Students/Staff BUILT, Family FAILED/missing_source_column, Classes/Enrollments NOT_RUN; SD51 Family EMPTY/no_rows_after_transform (contacts file lacks the configured Email column — answers the plan's SD51 verify item: a missing COLUMN, not blank values) |
| S3 | **DONE — integrated** | `claude/0053-s3-partial-reader` → `claude/0053-etl-failure-policy` | `failure_copy.py` (one copy source), PARTIAL rung (inert until S4), category-aware FAILED copy on Home/Run History/Convert; `convert_error_copy` retired. Review: 10 findings all confirmed; 9 fixed as prescribed, 1 by the verifier's alternative (no `sftp_requested` record key — records pass `sftp_attempted and not sftp_ok`). Local: 8,092 passed / 98 skipped, cov 96.64%. Legacy records byte-identical (7-ledger sha256 compare vs base) |
| S4–S15 | not started | — | — |
...
**Integration PR:** `claude/0053-etl-failure-policy` → `main`, **DRAFT PR #146** (https://github.com/myblueprint-spaces/DistrictSync/pull/146) — keep its slice table current. **Next action:** start S4 (spec: plan § "S4 — The entity bulkhead"; effort xhigh).

**S3 findings the next slices need:** the failure tail is `failure_copy`'s two fixed strings chosen by a REQUIRED `delivery_requested` bool — records can only prove "not sent" via `sftp_attempted and not sftp_ok`, so a nightly with delivery configured that fails in the ETL reads "Nothing new was saved to your output folder" (a producer-side `sftp_requested` key would let it say more — not added); `classify_latest_reason`/`to_run_row` take a REQUIRED `prior_build` (walk-back); `ConvertResult.delivery_requested` is REQUIRED; `outcomes.failed_entities` returns FAILED `EntityOutcome`s; `OUTCOME_TIER` is `Mapping[OutcomeKind, Verdict]` (S8 widens it); if S4's partner docs quote any `failure_copy` string, add the parity pin there; S2's CI on #146 green (runs 36042014443/36042014875).

**S2 findings the next slices need:** `DataTransformer.data_errors_mark`/`rollback_data_errors` exist with NO production caller — S4 must use them or delete them; the creator-gate test stubs (`test_ui_flet_config_editor`, `test_ui_flet_creator_flow`, `test_app_config_creator`) pass `entity_outcomes=()`, which S4's gate would read as "nothing failed" — give them real outcomes in S4; a Convert transform raise still writes no record (S5) though its ledger is right; `configured_entity_order` now de-duplicates (DECISIONS); the ledger is built after the output pre-flight on both paths.
