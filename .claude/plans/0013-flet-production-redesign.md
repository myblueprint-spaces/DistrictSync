...
| DS-1 | 1 Design system | **LANDED (plan 0017)** — pure `verdict.py` (Verdict→visuals, non-colour-cue tested) + amber-`#B45309` warning (AA-gated) **decoupled from the M3 `secondary` role (now `MB_ACCENT`)** + `card`/button/`ErrorCard`/`HealthVerdictBanner` primitives adopted in shell/picker_field/setup (Save-gate disabled styling preserved) + dev-only render demo. **Deviation:** ships **error + content** state patterns; **empty/loading deferred to their first consumer (IA-3 Home / IA-6 tables)** per build-at-first-consumer. See DECISIONS 2026-06-30. | L/med | PLAT-1 |
