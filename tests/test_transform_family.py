...
class TestFamilyTransform:
    ...
    def test_basic_family_transform(
        self, emergency_contact_df, family_mapping, global_config, raw_data
    ):
        result = self.transformer.transform(
            emergency_contact_df, family_mapping, "Family", raw_data, global_config
        )
        assert len(result) == len(emergency_contact_df)
        for field in family_mapping["field_map"]:
            assert field in result.columns

    def test_maps_correct_fields(
        self, emergency_contact_df, family_mapping, global_config, raw_data
    ):
        result = self.transformer.transform(
            emergency_contact_df, family_mapping, "Family", raw_data, global_config
    ...
    def test_multiple_contacts_per_student(
        self, emergency_contact_df, family_mapping, global_config, raw_data
    ):
    ...
    def test_empty_contacts_returns_empty(self, family_mapping, global_config):
        ...
        result = self.transformer.transform(
            empty_df, family_mapping, "Family", raw_data, global_config
        )
        assert result.empty
    ...
    def test_missing_email_produces_na(self, family_mapping, global_config):
        ...
        result = self.transformer.transform(
            no_email_df, family_mapping, "Family", raw_data, global_config
        )
        assert len(result) == 1
    ...
    def test_names_with_special_characters(self, family_mapping, global_config):
        ...
        result = self.transformer.transform(
            special_df, family_mapping, "Family", raw_data, global_config
        )
    ...
    def test_column_name_case_insensitive(self, family_mapping, global_config):
        ...
        result = self.transformer.transform(
            upper_df, family_mapping, "Family", raw_data, global_config
        )
        assert len(result) == 1
        assert result["First Name"].iloc[0] == "John"
