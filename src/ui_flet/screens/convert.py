...
**Scope (IA-5a):** local convert only — NO SFTP delivery leg and NO write-in-flight
close guard (both are IA-5b). A local convert with no SFTP configured is a
legitimate full flow (the Streamlit SFTP step is likewise conditional).
"""
...
# --------------------------------------------------------------------------- #
def convert_job(config_name: str, input_dir: str, *, anomaly_ack: bool = False) -> ConvertResult:
    ...
    when clear or acknowledged — ``save_all`` + the quality report. When a >20%
    anomaly fires without ``anomaly_ack`` it returns ``NEEDS_ANOMALY_ACK``
    **without writing**. ETL-level failures (a missing field-map column →
    ``save_all``'s ``ValueError``, ``load_config``'s errors) propagate as exceptions
    → the runner's ``on_error``. NO DataFrame is returned (privacy).
    """
...
def build_convert(page: ft.Page) -> ft.Control:  # pragma: no cover - Flet view glue
    ...
    def _start_convert(*, anomaly_ack: bool = False) -> None:
        ...
        started = runner.run(
            page,
            lambda: convert_job(district, input_dir, anomaly_ack=anomaly_ack),
            on_done=_on_done,
            on_error=_on_error,
        )
