...
- `screens/home.py` — health dashboard: one plain-language verdict ("did the roster sync?") + metric tiles + off-thread schedule read-back (D4), or the first-run onboarding hero (single front door) when unconfigured. **Three identity cards (0038 S4b, LIVE)** ride the configured branch immediately under the verdict block — the anchor is deliberate (it survives S7's tile subtraction): (1) the one-time upgrade ask (`identity_gate.needs_identity_prompt` — readable ∧ setup finished ∧ not dismissed ∧ no usable stored address; mutually exclusive with `needs_identity`, so nobody is asked twice), (2) the G3 mismatch card (session-local, only after a resolution whose match set EXCLUDES `sis_type`; Keep dismisses, "Change district"→Mapping; writes NOTHING either way), (3) the durable not-listed card (`unmapped_sd_number`; "Email support" reuses the untouched subject-only `about.support_mailto`, and its dismiss clears `identity_sd_number` via `identity_save` — never `identity_clear`, whose quarantine purge is an erasure this is not). All writes route through `identity_save`, so no card can touch `sis_type`; the cards carry their OWN floor above Home's `ErrorCard` (an advisory bug may never replace the verdict), and the 11-YAML domain index is built lazily in the Save handler, never on mount. Copy is IMPORTED from the S4a constants wherever the fact is identical — including the headline, which is why "is the launch page showing?" is now asserted structurally (no rail + a Continue button), not by that string.
