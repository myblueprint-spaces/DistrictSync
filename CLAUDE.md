...
- `src/ui_flet/identity_gate.py` — COUNTED pure gate + matching (no flet, no I/O). `needs_identity(cfg)` = readable settings ∧ setup unfinished ∧ no USABLE stored identity (mirrors `nav.needs_setup`); `stored_identity_email(cfg)` re-validates the stored value at READ time (`config.json` is hand-editable and the load check is TYPE-only — a failure means UNANSWERED, never echo it); `gate_reason` (bounded PII-free log word); `can_continue`; and the **S4a→S5 seam** `resolve_domain`/`matched_state`/`resolve_sd_number` — EXACT case-normalised domain equality, never subdomain/suffix (over-matching is the dangerous direction under fail-open). LIVE since S4a: `shell.main` gates on it and `screens/identity.py` resolves through it. It can only ask a question — it can never withhold anything and must never fail closed. The effectful index it consumes is `mapping_catalog.district_domain_index` (S5 replaces that with `filtered_catalog`; the pure resolver is unchanged by the swap).
