...
### Validate configs
```bash
make validate-config  # validates all 12 configs: myedbc, sd40, sd48, sd51, sd54, sd60, sd74, sd83, mbp_all, mbp_core, mbponly, sd51attendance
```
...
Native Flet 1.0 desktop app (no browser). `main.py`'s no-argv branch launches it via `src/ui_flet/launcher.py` (`boot_logging()` + one-time `migrate_legacy_data_dir()`) → `shell.py`. **Boot order is enumerated in `shell.main`'s docstring and is load-bearing** (0038 S4a): chrome → geometry → close handlers (hoisted ABOVE the gate — the launch page has no rail and no Exit, so the title-bar close is its only exit) → `needs_identity` gate-or-`build_app_body(page, cfg)` → probes. `page.add(root_host)` runs exactly ONCE; the launch page and the app body swap via `root_host.content`; `_enter_app` is idempotent and re-loads `AppConfig` after the gate (persist-then-enter, so the first paint is correctly scoped). Any raise in the identity layer → log + enter unfiltered. The app body = themed window + brand icon + left `NavigationRail` in ONE FIXED order (Home, Convert, Run History, Setup, Mapping, Help; D7) with rail-follow on programmatic nav + a schedule-attention Setup badge. **Launch selection is Home in EVERY state since S6** (`nav.initial_destination_id`; `_INITIAL_WHEN_SETUP_NEEDED` retired and `NavModel` collapsed to `destinations` — `nav_model()` takes no config, so D7's "same rail in every state" is structural rather than coincidental), and the Setup badge is **suppressed while `nav.needs_setup`** (`needs_setup_badge(setup_unfinished=…)`) — an attention dot on the rail item whose wizard is on screen would flag the work in progress as a fault. Seven surfaces in `screens/`:
- `screens/identity.py` — **the launch page** (0038 S4a, LIVE): "who looks after this sync?" → one work email → EXACT public-domain match → matched-one (a correctable pre-selection) / matched-several / calm no-match (+ SD number → not-listed). The ONE `hero_gradient()` surface in the app (reallocated from onboarding) and — since 2026-08-04 — the ONE CENTRED one: `shell._gate_frame` centres a `GATE_WIDTH` column on both axes, the cards STRETCH to it, and the field carries no helper, no counter and no `max_length` (the length rule stays at the `validate_identity_email` boundary). Two 0.85.3 traps behind that, both in `docs/FLET_1.0_CONVENTIONS.md`: a scrollable `Column` silently ignores `horizontal_alignment`, and a blank `counter=` does not collapse the counter's band. Every path enters the app, incl. the text-tier escape ("I'm not the person who set this up" — stores nothing) and a crash. NO `ErrorCard` floor by design (it would be a dead end in front of the app; the shell's floor enters instead). The module docstring carries the "deliberately absent" list (no lockout / counter / delay / lock glyph / password field / network state) with reasons — read it before adding anything to this page.
- `screens/home.py` — **slim** health dashboard (0038 S7): one plain-language verdict ("did the roster sync?") whose HEALTHY detail carries ONE roster-size number + a quick-action strip + off-thread schedule read-back (D4), or — while `nav.needs_setup` — **Home HOSTS the setup wizard itself** (0038 S6, `_wizard_host`): `build_setup(page, on_schedule_changed=…, on_complete=…)` under ONE state-aware `home_status.welcome_band` line — **FOUR** variants, never "Welcome" over a year of records and never a step count (the wizard's own indicator owns that): fresh → `WELCOME_FRESH`; readable run history → `WELCOME_RESUME_WITH_HISTORY`; saved choices (alone, or beside a store that would not OPEN) → `WELCOME_RESUME_SETTINGS_ONLY`; **nothing saved AND an unreadable store** → the artefact-free `WELCOME_RESUME_PLAIN`. The rule is *never name an artefact you have checked is absent or cannot see*, and it binds the SETTINGS as tightly as the store — which is why the fourth exists. The three doc copies of these literals are tied back by `tests/test_ui_flet_band_copy_parity.py` (a fifth variant quoted in a doc without registering it there is RED). Behind a branch-(a)-SPECIFIC `ErrorCard` floor (the dashboard floor's "your nightly sync keeps running" is FALSE pre-setup; this one says nothing was changed + routes to Help). `on_complete` re-enters via `on_navigate("home")`, so the shell's fresh `AppConfig.load()` is what routes to the dashboard — it fires ONLY after a verified save, which is why a failed finish can't bounce anyone to step 1. `screens/onboarding.py` is RETIRED (the hero was a door into the room you were standing in). **S7 (slim Home):** the metric-tile row + its "Latest roster" label are GONE (`components.section_label` retired with them — Home was its only caller); the one thing they carried that the verdict does not — *does the roster look the right size?* — is now the healthy detail's `home_status.size_clause`, keyed on `mapping_catalog.active_output_entities(sis_type)` (ONE memoised config read per mount, never the eleven-YAML `catalog()` build — the S4b cost note still holds, pinned). The clause names whatever entity THIS config produces (students / attendance rows / courses) and **vanishes** rather than guess, because the run record writes every entity key with a defaulted `0` and cannot tell "not enabled" from "collapsed to zero" — a non-zero filter would have hidden the exact alarm. It ALSO vanishes when the counts record's own `sis_type` differs from the saved district (`size_clause(..., *, expected_sis_type)`, required kwarg, applied to what `_counts_source` RETURNS — the delivery walk-back can land on a third district): Mapping's Apply rewrites `sis_type` without re-registering the task and Convert can run a district it never saved, so the entity list and the counts legitimately diverge, and pairing them printed "It included 0 students." under a GREEN band. Beneath the verdict block sits `_quick_actions_row` (Convert / Run History / Settings, minus the destination the fix CTA owns); tiering is decided by the pure `home_status.quick_actions`, so **exactly one filled primary exists in EVERY dashboard state** (the healthy branch had zero before). Per-entity visibility moves to Run History one click away for the rostering + myBlueprint+ entities; an **attendance** district's row count reaches exactly ONE surface — that healthy size clause — since Run History's columns exclude `StudentAttendance`, and none at all on a warning/failed run (the clause is HEALTHY-branch only). An OPEN ROADMAP item, not a discharged residual. **Three identity cards (0038 S4b, LIVE)** ride the configured branch immediately under the verdict block — the anchor is deliberate (it survived S7's tile subtraction unchanged): (1) the one-time upgrade ask (`identity_gate.needs_identity_prompt` — readable ∧ setup finished ∧ not dismissed ∧ no usable stored address; never both asks from ONE settings state — the launch-page decliner stores nothing, so finishing setup legitimately re-asks once here, dismissible forever, tracked in ROADMAP), (2) the G3 mismatch card (session-local, only after a resolution whose match set EXCLUDES `sis_type`; Keep dismisses, "Change district"→Mapping; writes NOTHING either way; its detail names what WAS saved, since it renders after a successful write), (3) the durable not-listed card ("We don't have a mapping for SD## yet" — never a claim about vendor work nobody was told about; `unmapped_sd_number` over `available_configs()`, so it never builds the domain index; the `Email support@…` button + the selectable/copyable address beside it — `help.py`'s house pattern, because a bare mailto is a dead click on a locked-down server; dismiss clears `identity_sd_number` via `identity_save` — never `identity_clear`, whose quarantine purge is an erasure this is not). All writes route through `identity_save`, so no card can touch `sis_type`; card headlines are capped at the verdict's `type_section`/W_700 (type hierarchy IS verdict-first); the cards carry their OWN floor above Home's `ErrorCard` (an advisory bug may never replace the verdict). Copy is IMPORTED from the S4a constants wherever the fact is identical and written fresh where it differs (Home has no "next step" and nothing pending to "choose") — the shared headline is why "is the launch page showing?" is now asserted structurally (no rail + the launch-page-only escape label), not by that string.
- `screens/setup.py` — **"Who looks after this sync" leads the Settings scroll** (0038 S4a): the stored address shown plainly (only after `stored_identity_email` re-validates it), Change re-runs the SAME resolution inline, **blank CLEARS** via `identity_clear` (both fields + `identity_prompt_dismissed` reset; the `config.corrupt-*.json` copies swept only when something WAS stored, and `identity_cleared_note` branches on the real outcome so a failed unlink is never reported as a deletion). Adds no filled primary and no reconcile — identity is not a task-baked arg. Also: **first-run 5-step WIZARD** (District → Folders → Delivery → Schedule → honest adaptive finish; District leads per 2026-07-15 user decision, Delivery still precedes Schedule so `--sftp` is baked before registration); Schedule + Delivery are skippable. Run time uses a `ft.TimePicker` clock affordance (the TextField stays the value source of truth). Once completed, graduates to a flat **Settings** scroll (rail label stays "Setup") ordered folders/district → schedule → delivery + ONE reconciling Save (re-registers when any task-arg changed). **S6:** `build_setup` takes `on_complete` — the HOST seam. `_finish` is save-THEN-verify: a raised `cfg.save()` rolls `setup_completed` back on the instance, keeps the finish summary on screen with `FINISH_SAVE_FAILED_NOTE` (retry = press again), logs WARN, and does NOT fire `on_complete`; with `on_complete=None` (the Setup RAIL item) the SUCCESS path is unchanged (the in-place `_mount_settings(transition_cue=True)` graduation) while the FAILURE path is deliberately NEW on BOTH mounts — a raised save used to escape uncaught. Pinned by a text/label equality test over the two mounts, because a rail item and a hosted wizard that diverge are two wizards. Schedule → per-op UAC-elevated `register_task`; SFTP → side-effect-free test + keyring-on-Save-only. Neither password persisted to `AppConfig`. Only the wizard finish line flips `setup_completed`. **S5:** both district dropdowns (wizard step + Settings folders card) are `filtered_catalog`-scoped — **unconditionally since 2026-08-04**, when the owner retired the `list_scope_row` show-all row from all four pickers (the escape moved to the INPUT: blanking the stored address in this same scroll widens every list); the wizard's auto-select seed reads the VISIBLE list and is applied **AFTER** `derive_flow` — order is load-bearing, since seeding it first marks the District step satisfied and resumes PAST the step the launch page just promised. **The seed fires ONLY when nothing is saved** (a resumed wizard with a district already set skips the step entirely, as it always has), and when it fires the step's caption SAYS SO (`district_auto_seeded_note`) instead of instructing a pick already made — a pre-selection nobody is told about is a silent default.
- `screens/convert.py` — run a conversion now on a worker thread (`job_runner.py`); explicit district (no `configs[0]` fallback, no input-dir fallback — fails loud on unset output); anomaly-ack write-gate; resolved-output caption + "Open folder"; optional SFTP delivery; writes a `source="manual"` run record. **S5:** the district dropdown is `filtered_catalog`-scoped (the prefill membership test moved to the VISIBLE ids — safe because the saved district always rides the list; no show-all row since 2026-08-04) and the header gained a `this_run_label` "This run: <district>" pill + "Change mapping" that appear on DIVERGENCE only — a label, never a gate (`can_run_convert` untouched).
- `screens/run_history.py` — read-only past runs from the SQLite run store (`src/history/store.py`), NOT log parsing; plain-language status (no raw error/path column); honest fresh-start empty state after this update. **Its empty-state RULE and both headlines are imported from `home_status` (S7)** — `has_earlier_run_history(store_created_at=…)` alone, never `has_completed_setup()` (the wizard flips that on save, which told every brand-new install about "an earlier version" it never had). The stamp cannot separate "never ran" from "ran before `history.db` existed" (shipped v3.5.0, no backfill), so `EMPTY_NO_RUNS_HEADLINE` claims only the LEDGER ("No runs recorded yet") and no empty-state sentence on either surface NAMES a nightly without a positive signal — Home gates its nightly-naming leads on `home_status._expects_a_nightly` (confirmed-LIVE read-back OR `schedule_registered`, and its lead says "your nightly sync", never "your FIRST": that gate admits the ≤v3.4.0 upgrader by design); **BOTH** of this banner's leads simply name none (the upgrade arm is gated on the store stamp ALONE, so a manual-only install reaches it — it says "New **runs** will appear here"), and the nightly is named only by the confirmed-LIVE append. Both leads are pinned by exact-string rows in `tests/test_ui_flet_run_history.py` — they were referenced nowhere until the S7 discharge round, so either could revert green. The LEAD sentences stay per-surface by design; the parity sweep asserts VERDICTS on the record axis and headline equality only on the empty state — `tests/test_ui_flet_home_history_parity.py`.
- `screens/mapping.py` — review the active district config and switch to another pre-built one (NOT a YAML editor — the full editor is a ROADMAP item). **S5:** ONE `filtered_catalog` build per mount (the old double `list_configs()` parse is gone); the current mapping is always offerable, so a switch is always revertible. Since 2026-08-04 there is NO show-all row, so a matched admin switches only between their own district's tiers.
- `screens/help.py` — links out to the SpacesEDU Help Centre + support email; plus the read-only "who looks after this sync" echo (0038 S4a — rendered ONLY when the stored address re-validates; carries "we don't send it anywhere" because `about.support_mailto` stays subject-only — flag 6) + a one-click hop to Settings.

Pure COUNTED (tested) logic: `tokens`/`theme`/`verdict`/`nav`/`humanize`/`home_status`/`schedule_status`/`schedule_probe`/`convert_result`/`convert_output`/`run_history`/`mapping_catalog`/`identity_gate`/`setup_errors`/`setup_gates`/`sftp_copy`/`setup_flow`/`job_runner` (the retired `run_log` parser is replaced by the `history.store` reader); view glue (`shell`/`nav_rail`/`components`/`picker_field`/`launcher` + `screens/*`, incl. `screens/identity.py`) is coverage-omitted (`screens/onboarding.py` left the omit list with the module — S6). Build all controls via `components.py` (see `docs/FLET_1.0_CONVENTIONS.md` — e.g. `ft.Dropdown` uses `on_select`, `ft.FilledButton` uses `content=`; the wrong forms raise `TypeError` on 0.85.3).
...
### Supporting modules
- `src/etl/sync_window.py` — **Seasonal sync window** (v3.8.0): pure `today`-injected `in_sync_window(today, start_md, end_md)` (MM-DD bounds, wrap-around Aug→Jul, boundary-inclusive, leap-safe via `(month, day)` tuple compare — never builds a date, so `02-29` can't raise) + `next_resume_date`. The window lives in the APP, NOT the OS schedule: the task stays a plain daily trigger and `main._cli` skips a run whose resolved source is `scheduled` when the window is enabled and today is outside it (exit **0** — a paused night is healthy, not a failure; no ETL/write/delivery, no run record). So it recurs yearly with **nothing to re-arm** — no scheduler change, no elevation. Opt-in (`AppConfig.sync_window_enabled` default off → year-round, byte-identical). Manual Convert / hand-run CLI / headless cron BYPASS. A malformed enabled window fails LOUD and runs (never silently stops the sync). **Fields are named `sync_window_*` deliberately — `window_*` is the geometry-exclusion prefix in `app_config._carries_chosen_settings`.** The paused fact is single-sourced at `ui_flet/home_status.sync_window_paused` and read by Home, the Run History banner AND the Setup nav badge so they cannot disagree; a confirmed-MISSING schedule OUTRANKS the pause (a gone task won't resume, so "resumes <date>" would be a lie).
- `src/config/app_config.py` — Runtime config (`config.json` in `paths.user_data_dir()`); SFTP non-sensitive settings + `setup_completed`. `has_completed_setup()` (D4a) is the SINGLE durable finish-line (explicit flag OR `is_complete() and schedule_registered`, baked on load so no deployed install regresses into onboarding); `sis_type` defaults to `""` (D9 — no silent district). Unix file permissions (0o700/0o600). **Identity (0038):** `identity_email`/`identity_prompt_dismissed`/`identity_sd_number` — the `identity_` prefix is LOAD-BEARING (it puts them in `_ADVISORY_FIELD_PREFIXES` alongside `window_`, so an identity-only save on an UNREADABLE profile is refused by the existing `_carries_chosen_settings` machinery). `identity_save(**updates)` is **the ONE sanctioned identity write path** (review-maintained — nothing mechanically prevents a direct attribute write + `save()`): re-checks `settings_unreadable()` at WRITE time, validates KEY *and* VALUE before applying any of them (membership in the dataclass-derived `_IDENTITY_FIELD_NAMES`, never `hasattr` — which would let `identity_save=...` shadow the method — plus `_value_fits`, because a mis-typed value makes the whole `config.json` UNREADABLE on next load), refuses any non-`identity_*` key loudly (so it can never rewrite `sis_type`), swallows+logs `SettingsOverwriteRefused`/`OSError`, ROLLS THE INSTANCE BACK (the AppConfig is SHARED across every Settings section — a refused write left applied would render state the disk lacks and get committed by the next unrelated Save) and returns a bool (advisory data must never trap the admin). `identity_clear()` (0038 S4a) is the ERASURE path → `ClearOutcome(cleared, removed, remaining)`: routes through `identity_save` (blanks both fields + resets `identity_prompt_dismissed` so the ask can return), then unlinks the `config.corrupt-*.json` siblings — gated on BOTH a confirmed write AND `had_identity` read BEFORE it (a blank Save on an install with nothing stored must not destroy the settings-recovery copies of the very population most likely to own them). `remaining` exists so the note can't claim a deletion that failed; a stranded `.config.json.*.tmp` is a named residual no sweep reaches.
- `src/utils/identity.py` — COUNTED identity primitives (0038): `normalize_email` (trim→lowercase→NFC; plus-addressing KEPT, exactly ONE trailing root dot stripped) + `extract_domain` (after the FINAL `@`). **No salt/hash/scheme** — matching compares PUBLIC district domains. Posture inversion vs `ALLOWED_SFTP_HOSTS`: same normalize-then-compare shape, OPPOSITE failure mode — TOTAL and must never narrow to zero (consumers fail OPEN). Boundary validation is `validators.validate_identity_email` (returns the value UN-normalised; its messages never echo the address). Domain matching is EXACT string equality — never subdomain/suffix.
- `src/ui_flet/identity_gate.py` — COUNTED pure gate + matching (no flet, no I/O). `needs_identity(cfg)` = readable settings ∧ setup unfinished ∧ no USABLE stored identity (mirrors `nav.needs_setup`); `stored_identity_email(cfg)` re-validates the stored value at READ time (`config.json` is hand-editable and the load check is TYPE-only — a failure means UNANSWERED, never echo it); `stored_identity_domain(cfg)` (S5) is the ONE reduction every filtered picker consumes — `""` when unusable **or** when `settings_unreadable()` (that state has no trustworthy identity AND no saved district to fall back on, so it must not narrow anything); `gate_reason` (bounded PII-free log word); `can_continue`; and `resolve_domain`/`matched_state`/`resolve_sd_number` — EXACT case-normalised domain equality, never subdomain/suffix (over-matching is the dangerous direction under fail-open); the index is taken as DATA, which is why S5 could swap its source without touching the resolver. S4b adds the three Home-card rules: `needs_identity_prompt` (the same ask on a CONFIGURED install — the ONE input it adds is `identity_prompt_dismissed`; swept as mutually exclusive with `needs_identity`), `matched_excludes_saved` (G3 — deliberately narrow: no match and no configured district are both non-questions, never a disagreement) and `unmapped_sd_number` (total over a hand-edited value; a number we DO ship returns `""`, so we never claim to be "building" a mapping that is in the exe). LIVE since S4a: `shell.main` gates on it, `screens/identity.py` resolves through it, `screens/home.py` cards read it. It can only ask a question — it can never withhold anything and must never fail closed. Since S5 the effectful side is ONE session-memoised `mapping_catalog.catalog()` build, projected as `district_domain_index` (identity surfaces) or consumed by `filtered_catalog` (the four pickers).
...
`global_config.class_rostering_grades` (opt-in, plan 0042 1a; `Literal["homeroom"] | list[str] | None`, default `None` ⇒ today's behaviour byte-identical) is the **COMPLETE** set of grades that get class rostering, in **CEDS OUTPUT space** (`KG`/`01`… — never raw `"K"`/`"3"`; valid set = `grades.CEDS_GRADE_CODES`, derived from `CEDS_MAPPING.values()`, and `"Other"` is its one mixed-case member so nothing may case-normalise the codes). `homeroom_grades ⊆ class_rostering_grades` is VALIDATED, which is *why* the homeroom path needs no change: homeroom scope = `homeroom_grades`, timetable scope = the difference, grades in neither get NOTHING. `"homeroom"` is sugar for "rostered == homeroom_grades" (empty timetable scope) — SD83's shape, the only shipped consumer, `version: '1.10'` (quoted: PyYAML collapses bare `1.10` → `1.1`). Four boundary rules fire only when the key is set (`GlobalConfig.check_rostering_grade_scopes` + the `mode="before"` shape validator): CEDS shape · non-empty list · subset · **`homeroom_grades` itself CEDS-validated** (under the sentinel it *becomes* the rostered set, so nothing else would check it; general `homeroom_grades` validation stays a roadmap gap). The scope is derived by ONE function, `grades.resolve_timetable_scope`, called at each of its three consumers (`classes.py`/`enrollments.py`/`blended.py` — pure, so they cannot disagree) (`None` = no scope in force — branch on `is None`, NEVER truthiness: an empty set means "roster no timetable classes") and applied INSIDE `split_by_homeroom_grades(keep="subject")`, the one place the grade→CEDS conversion happens — because `grade_to_ceds` is **not idempotent** (`KG`/`IT`/`PR`/`PK`/`PS` are values, not keys, so a second pass yields `"UG"`). `keep="homeroom"` + `timetable_scope` **RAISES** (an ignored filter argument is the banned permissive default). `blended._register_blends` drops a blend whose grades miss the scope entirely — **before any `result.*` write**, since `class_map`/`teacher_map` are written before the grade range is known and a later `continue` would leave orphan Class IDs in `Enrollments.csv` (the `e187ac8` partner-ingest rejection). The rule is necessary-not-sufficient (`_build_grade_map` keys MODE grades, so a mode-masked section falls back to its plain per-section class — no orphan) and stays GATED: an all-homeroom blend still ships for the other districts until plan 0043 ungates it.

Base `myedbc` defines all 7 entity templates; configs select which to emit via `global_config.enabled_entities` (see **Output Targeting**). 7 SpacesEDU district configs — `myedbc` (base), `sd40myedbc` (New Westminster — CSV files, headerless schedule), `sd48myedbc` (Sea to Sky), `sd51myedbc` (Boundary), `sd54myedbc` (Bulkley Valley), `sd60myedbc` (Peace River North), `sd74myedbc` (Gold Trail) — each `_base: myedbc` and inherit the 5 rostering entities. `sd83myedbc` (North Okanagan-Shuswap) is an 8th, also `_base: myedbc`, but opts into the full myBlueprint+ tier (all 7 entities) with district-specific overrides: `homeroom_grades` extended through 08, `class_rostering_grades: "homeroom"` (K-8 classes only; 9-12 stay ROSTERED so their transcripts work), `course_start_grade: 9`, and Date of Birth withheld (`{value: ""}`) — standard MyEd BC file naming assumed pending real GDE samples. 3 myBlueprint+ tier configs — `mbp_all` (all 7), `mbp_core` (Students + CourseInfo + StudentCourses), and `mbponly` (CourseInfo + StudentCourses only) — also `_base: myedbc`, overriding `enabled_entities`.

`district_domains` (top-level, beside `district_name`) is the district's PUBLIC staff email domain list — PRESENTATION metadata. **Read since S4a** by `mapping_catalog.district_domain_index` → the launch page + the Settings identity section, and since S4b the Home ask + mismatch cards (which build the index LAZILY, on Save only — never on mount; the not-listed card is domain-free, matching an SD number against `available_configs()`). **The PICKERS ARE SCOPED since S5** via `mapping_catalog.filtered_catalog` (wizard District step + its auto-select seed · Settings folders card · Convert · Mapping), and **UNCONDITIONALLY since 2026-08-04** — the per-surface "Show all districts" row and its `components.list_scope_row` factory, the `show_all` parameter and `FilteredCatalog.can_filter` are all DELETED (owner decision; do not re-add a widening control). Scoping is visibility, never access: the filter still fails OPEN everywhere (no identity / no match / unreadable catalog / any raise → the full list), the saved + picked districts still ride every list, and the ONE escape is blanking the stored address in Settings. Structurally invisible to the ETL (`to_raw_dict` emits only `mappings` + `global_config`). Seven shipped rows: sd40/48/51/54/74/83 at `sd##.bc.ca`, SD60 at `prn.bc.ca` (the STAFF domain, deliberately NOT its generated student domain); base + `mbp_*` carry none (unclaimed = shown in every state); `sd51attendance` INHERITS SD51's via `_base`. A field validator rejects anything that isn't a lowercase domain — a pasted email address fails `make validate-config` LOUDLY. `MappingConfig` declares `extra="ignore"` explicitly (forward-compat: a root key from a newer build must not fail a district's sync); exactly five leaf models forbid extras (`EmailDerivedDate`, `FieldEmailFormat`, `FieldEnrollStatus`, `RowFilter`, `CrossEnrollmentConfig`) — `GlobalConfig`/`EntityConfig`/`FieldTransform`/`FieldNameConfig` do NOT, which is why a typo'd `enabled_entities` is silently dropped. Both directions pinned by `tests/test_config_district_domains.py`. **No `version:` bump for a presentation key** — the loader's bump-MINOR convention is scoped to ETL-affecting keys (2026-07-29 decision).
...
- **Structured logging + run store** — after each run the record is built ONCE → both the `__DISTRICTSYNC_RUN__` diagnostic log line (timing, counts, SFTP status, rich free-text error detail) AND the durable SQLite run store (`src/history/store.py`, bounded `error_category` only — privacy split). The store is the Run History source; the log stays ops diagnostics. Store writes are best-effort/non-fatal and never alter the exit-code contract. **`dry_run` writes NO store record on any path** (success/failure/early-exit — the gate sits at `_store_run_record`, the **pipeline's** single store sink, with `dry_run` required keyword-only so no path can forget it): a preview writes no files, so it must not paint a sync on Home or add a Run History row; its log line is unchanged. Scope: `screens/convert.py` (`_record_manual_run`) calls `write_run_record` directly and so bypasses that gate — Convert has no preview mode, and would need its own gate if one is added
...
- CI: ruff check + ruff format + mypy (non-UI) + bandit + pytest (80% coverage gate) + config validation — every config `available_configs()` discovers, with the count **pinned at 12** so a deleted/unregistered config fails loudly (keep the pin in lockstep with the Makefile's `validate-config` list)
