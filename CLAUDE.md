...
- `src/ui_flet/identity_gate.py` — Pure `needs_identity(cfg)` (no flet): readable settings ∧ setup unfinished ∧ no stored identity. Mirrors `nav.needs_setup`; identification for list-scoping, never a gate that can fail closed.
...
- `docs/claugentic-CHARTER.md` — per-work-TYPE record of the approach this project has settled on (apply / record / adapt / grow; a revisable default, not a mandate)
