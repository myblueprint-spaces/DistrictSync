...
- `src/ui_flet/identity_gate.py` — Pure `needs_identity(cfg)` (no flet): readable settings ∧ setup unfinished ∧ no stored identity. Mirrors `nav.needs_setup`. **No caller yet — S4a mounts it**; it will gate the launch page, which is identification for list-scoping, never a gate that can fail closed.
